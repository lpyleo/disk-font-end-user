<template>
  <div class="chat-app-warp">
    <UserLogin ref="login" v-if="!loginUser.id" @login="userLogin" :type="deviceType" v-drag></UserLogin>
    <div class="app-main-panel ui-clear" v-drag v-if="loginUser.id && deviceType === 'pc'">
      <div class="app-aside-panel">
        ...
        <ul class="aside-menu-list" @mousedown.stop>
          <li v-for="(item, i) in menus" :key="i" @click="curMenu = item.name" :class="{ active: curMenu === item.name }">
        ...
        <div class="app-container-panel ui-clear" v-show="curMenu === 'chat'">
          <div class="app-user-panel">
            ...
            <div class="app-users-warp scroll">
              <div @mousedown.stop>
                <UserItem class="app-user-item" v-for="(item, i) in searchUser(keyword)"
                  :class="{ active: item.id === curSession.id }" :user="item" :num="unReadNum(item.id)"
                  :message="finallyMessage(item.id)" @click.native="changeSession(item)" :key="i">
          ...
          <div class="app-session-panel">
            <UiSessionPanel :session="curSession" @sendMessage="sendMessage" v-if="curSession.id">
              ...
              <div class="message-list-warp scroll" slot="body" ref="message-list">
                <UiChatBubble class="message-item" :key="i" :message="item" :setting="setting"
                  :is-send="loginUser.id === item.from.id" v-for="(item, i) in getMessages(curSession.id)">
                </UiChatBubble>
              </div>
            </UiSessionPanel>
    ...
    <div class="app-main-touch" v-if="loginUser.id && deviceType === 'phone'">
      <div class="app-iChat-container">
        <div class="app-iChat-panel" v-show="curMenu === 'chat'">
          ...
          <div class="iChat-users-warp">
            <UserItem class="iChat-user-item" v-for="(item, i) in searchUser(keyword)"
              :class="{ active: item.id === curSession.id }" :user="item" :num="unReadNum(item.id)"
              :message="finallyMessage(item.id)" @click.native="changeSession(item)" :key="i">
              <template slot="time" v-if="finallyMessage(item.id).time">
                {{ finallyMessage(item.id).time | friendlyTime }}
      ...
      <div class="app-iChat-footer">
        <ul class="app-iChat-menus ui-clear">
          <li class="iChat-menu-item" v-for="(item, i) in menus" @click="curMenu = item.name"
            :class="{ active: curMenu === item.name }" :key="i">
...
import { getDeviceType } from "./JS/emoji";
import Message from "./JS/Message";
import { Manager } from 'socket.io-client';
import { friendlyTime, formatTime } from './JS/filters'
import { BELL_URL } from "./JS/config";
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      login.login(this.$store.getters.username, this.$store.getters.userAvatar);
    });
  },
  methods: {
    searchUser(keyword) {
